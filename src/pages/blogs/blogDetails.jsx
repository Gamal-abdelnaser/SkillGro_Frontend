...
export default function BlogDetails() {
  const { selectedBlogs,setSelectedBlogs } = useContext(BlogContext);
  
    useEffect(() => {
      const storedBlog = localStorage.getItem("selectedBlog");
      if (storedBlog) setSelectedBlogs(JSON.parse(storedBlog));
    }, [setSelectedBlogs]);
  
    useEffect(() => {
      if (selectedBlogs) {
        localStorage.setItem("selectedBlog", JSON.stringify(selectedBlogs));
      }
    }, [selectedBlogs]);
    ...
    <Stack as={Flex} w={'100%'} py={'10px'} alignItems={'center'} justifyContent={'center'} gap={1} >
      <Stack w={'full'} py={{base:'50px',md:'100px'}} mt={'80px'} alignItems={'center'} justifyContent={'center'}  bg={'#161439'} px={6} gap={{base:3,md:8}} >
        <Stack maxW={'1500px'} w={'100%'} px={6} gap={{ base: 3, md: 8 }}>
          ...
          <Flex alignItems={'center'} textAlign={'start'} gap={2} color="#fff"  w={'full'}>
            <Link color="#fff" to={'/'}>Home</Link> <IoIosArrowForward color="#7f7e97" size={12} /> <Link color="#fff" to={'/'}> Blogs</Link> <IoIosArrowForward color="#7f7e97" size={12} /><Text color="#726de7" > {Blog?.title}</Text>
          </Flex>
        </Stack>
      </Stack>
      <Flex maxW={'1500px'} w={'100%'} alignItems={'center'} m={0} justifyItems={'center'}>
        <Flex position={'relative'} w={'98%'} pt={'100px'} alignItems={{ md: 'start', base: 'center' }} justifyContent={'space-between'} flexDirection={{ base: 'column', md: 'row' }} mx={'auto'}>
          <LeftContainer resData={Blog}  />
          <Rightcontainer resData={Blog} />
        </Flex >
      </Flex>
    </Stack>
